<template>
  <div class="container">
    ...
    <!-- <el-breadcrumb separator-class="el-icon-arrow-right" >
      <el-breadcrumb-item :to="{ name: 'search'}">返回</el-breadcrumb-item>
      <el-breadcrumb-item>详情页面</el-breadcrumb-item>
    </el-breadcrumb> -->
    <div class="main">
      <div class="contain loading">
        <div class="leftPicture" @mouseover="enter" @mouseout="leave" @mousemove="move" ref="picture">
          <img :src="'http://47.107.235.13:80'+ detailList.itemPicture"/> 
           <div class="small_box" ref="smallBox"  >
          </div>
    ...
    <comment ref="comment" v-on:reload="getReply"></comment>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 0：未赞  1：已赞
    thumbsUpPar(item) {
      item.state === 0?item.thumbsUp ++: item.thumbsUp --
      return {
        id: item.id,
    ...
    thumbsUpCom(item) {
      ...
      this.$http.thumbsUpComment(this.thumbsUpPar(item)).then( r => item.state === 0? item.state = 1: item.state = 0)
    },
    // 点赞回复 并调整状态
    thumbsUpRep(item) {
      if(!this.isLogin()) return
      //  let params = this.thumbsUpPar(item)
       this.$http.thumbsUpReply(this.thumbsUpPar(item)).then( r =>  item.state === 0? item.state = 1: item.state = 0)
    },
    // 查看回复
    readMore(id,index) {
