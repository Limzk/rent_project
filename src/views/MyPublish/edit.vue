<template>
  ...
  >
     <el-tabs v-model="activeName" type="card">
         <el-tab-pane label="无需审核" name="first">
            <el-form :model="auditForm" ref="auditForm" label-width="100px" class="demo-dynamic">
            <!-- 表单提交内容 -->
                <el-form-item prop="price" label="价格" :rules="[{ required: true, message: '价格不能为空', trigger: 'blur' }]">
                <el-input v-model="auditForm.price" style="width:390px;" oninput="value=value.replace(/[^0-9.]/g,'')" ></el-input>
                <span>元/{{ auditForm.chargeType === "0"? "天":"时" }}</span>
                </el-form-item>
                <el-form-item>
                <el-radio-group v-model="auditForm.chargeType">
            ...
            <el-form-item  prop="deposit"  label="押金"    :rules="[{ required: true, message: '押金金额不能为空', trigger: 'blur' }]">
                <el-input  v-model="auditForm.deposit"  oninput="value=value.replace(/[^0-9.]/g,'')"  ></el-input>
            </el-form-item>
            <el-form-item class="dialog-footer">
                <el-button @click="cancel">取消</el-button>
                <el-button type="primary" @click="changeForm('auditForm')">提交</el-button>
            </el-form-item>
            </el-form>
         </el-tab-pane>
        <el-tab-pane label="需审核" name="second">
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$http.itemUpdate({
            ...
            chargeType: parseInt(this.auditForm.chargeType),
            deposit: this.auditForm.deposit,
            id: this.id
          }).then ( r => {
